...
class PaperScrollAPI {
	constructor(options: PaperScrollAPI) {
		if (!options.merchant_id) throw new ParameterError('merchant_id');
		if (!options.access_token) throw new ParameterError('access_token');

		this.api = new API(options.merchant_id, options.access_token);
	}
}
